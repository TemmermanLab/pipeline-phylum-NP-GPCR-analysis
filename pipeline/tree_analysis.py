...
def process_tree(fname, do_draw=False):
    ...
    for idx in tqdm(order, desc="Clustering Terminals"):
        ...
        while queue:
            ...
            other_terminals = [o for o in other_terminals if o.name not in cluster]
            all_clusters.append(cluster)

        cel_clusters[c.name] = {
            "cluster_hierarchy": all_clusters,
    ...
    for fname in tqdm(fas, "Searching Species"):
        ...
        for cluster in input:
            ...
            for c in current_cluster_keys:
                cels_vs_species[c][sname] = len(
                    [i for i in cluster[c]["cluster"] if i in entries_per_species]
                )

    cel_vs_species_list = [[cels_vs_species[c][s] for c in cels] for s in species]
    df = pd.DataFrame(cels_vs_species).transpose()
    species_fixed = [
        "{} {}".format(*re.split("[_.]", s)[:2]).capitalize() for s in species
    ]
...
def process_dist(tree, other_terminals, cel_terminals):
    dists = [
        [tree.distance(cel, t) for t in other_terminals] for cel in tqdm(cel_terminals)
    ]
...
if __name__ == "__main__":
    ...
    if not already_done:
        all_trees = [
            t for t in os.listdir(rootdir) if ".tree" in t and "colored" not in t
        ]
        all_trees_clusters = []
